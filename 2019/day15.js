  ...
  while(true) {
    ...
    switch (opcode) {
      ...
      case 2:
        ...
        break;
      case 3:
        log(`INP: tab[${getAddress(tab, pos, 1, parameters, base)}] = ${input}`)
        tab[getAddress(tab, pos, 1, parameters, base)] = input
        pos += 2
        break;
      case 4:
        log(`OUT: ${first}`)
...
let map = new Array(42)
for (let i = 0; i < map.length; i++) map[i] = new Array(map.length).fill(null);

let x = y = 22
let program = calc(tab)
program.next()
function changedir(x, y, dir) {
  switch(dir) {
    case 1: return {dir, x, y: y -1, back: 2}
    case 2: return {dir, x, y: y + 1, back: 1}
    case 3: return {dir, x: x - 1, y, back: 4}
    case 4: return {dir, x: x + 1, y, back: 3}
  }
}

function findAvailable(x, y) {
  let routes = []
  for (let dir of [4,3,2,1]) {
    let { x: x1, y: y1 } = changedir(x, y, dir)
    if (map[y1][x1] === null) routes.push(dir)
  }
  return routes;
}
...
async function play() {
  ...
  let path = []
  do {
    let available = findAvailable(x, y);
    if (available.length === 0) {
      if (path.length === 0) {
        break;
      }
      let last = path.pop();
      available.push(last.back)
      back = true
    }
    else {
      back = false;
    ...
    for (let dir of available) {
      let move = changedir(x, y, dir)
      if (!check(move.x, move.y)) {
        continue
      }
      next = program.next(dir)
      if (next.done) break;
      ...
      if (output == 0) {
        ...
        if (output == 2) {
          map[move.y][move.y] = "O"
          oxygen = { x : move.x, y: move.y, steps: path.length + 1 }
        }
        else {
          map[move.y][move.x] = "."
        }
  ...
  while (!next.done)
  display()
  console.log(`Minimum ${oxygen.steps} steps to find oxygen`)
  fillOxygen(oxygen)
}

function changeOxygen(x, y, minutes) {
  if (map[y][x] == '.') {
    map[y][x] = minutes + 1
    return 1;
  }
...
function fillOxygen(oxygen) {
  let minutes = 0;
  let propaged = 1;
  map[oxygen.y][oxygen.x] = 0;
  while (propaged > 0) {
    propaged = 0;
    for (let y = 0; y < map.length; y++)
      for (let x = 0; x < map[y].length; x++)
        if (map[y][x] === minutes) {
          propaged += changeOxygen(y - 1, x, minutes)
          propaged += changeOxygen(y + 1, x, minutes)
          propaged += changeOxygen(y, x - 1, minutes)
          propaged += changeOxygen(y, x + 1, minutes)
        }
    minutes++
  }
  console.log(`Oxygen fill all case in ${minutes-1} minutes`)
}

function printBlock(val) {
  return val || ' '
}
...
function display() {
  ...
  let save = map[y][x]
  map[y][x] = "X"
  console.log(map.map(tab => tab.map(printBlock).join('')).join('\n'))
  map[y][x] = save
}
