...
function buildMap(tab) {
    return tab.map(item => item.split(' contain ').map(l => l.replace('.', '').replace(/ bags?/g, ''))).reduce((map, [key, value])=> {
        map[key] = buildBag(value);
        return map;
    }, {})
}

function canContainBag(container, content, bags) {
    if (!bags[container]) {
        return false;
    }
    ...
    for (const containedBag of bags[container]) {
        if (canContainBag(containedBag.name, content, bags)) {
            return true;
        }
    }
    return false;
...
function findShiny(bags) {
    log(bags);
    let count = 0;
    for (const bag in bags) {
        log(bag)
        if (canContainBag(bag, 'shiny gold', bags)) count++;
    }
    return count;
}

function getContainedBagsAmount(container, bags) {
    log(container)
    ...
    for (const containedBag of bags[container]) {
        log(containedBag)
        containedBagsAmount += containedBag.number;
        containedBagsAmount += containedBag.number * getContainedBagsAmount(containedBag.name, bags);
    }
    return containedBagsAmount;
}
let bags = buildMap(tab);
console.log(`Result 1 : ${findShiny(bags)}`);
console.log(`Result 2 : ${getContainedBagsAmount('shiny gold', bags)}`);
