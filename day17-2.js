...
function read() {
  ...
  do {
    ...
    if (output === 10) {
      ...
      map[line].push(String.fromCharCode(output))
    }
  }
  while (!next.done)
  display(map)
...
function check(map, x, y) {
  if (y >= map.length || y < 0 || x < 0 || x >= map[y].length) return false;
  return map[y][x] === "#";
}

function countIntersection(map) {
  let total = 0;
  for (let y = 0; y < map.length; y++)
    for (let x = 0; x < map[y].length; x++) {
      if (check(map, x, y) && check(map, x - 1, y) && check(map, x + 1, y) && check(map, x, y - 1) && check(map, x, y +1)) {
        map[y][x] = "O";
        total += x * y
      }
    }
...
// chemin : R12L6R12 L8L6L10 R12L6R12 R12L10L6R10 L8L6L10 R12L10L6R10 L8L6L10 R12L10L6R10 R12L6R12 R12L10L6R10
...
async function run() {
  ...
  let A = 'R,12,L,6,R,12\n'
  let B = 'L,8,L,6,L,10\n'
  let C = 'R,12,L,10,L,6,R,10\n'
  let inputStr = (main + A + B + C+'y\n')
  let input = []
  for (let i = 0; i < inputStr.length; i++) input.push(inputStr.charCodeAt(i))
  ...
  do {
    ...
    if (next.value > 127) {
      output = next.value;
      console.log(String.fromCharCode(next.value))
    }
    else
    res += String.fromCharCode(next.value)
    if (res[res.length - 2] === res[res.length - 1] && res[res.length - 1] === '\n') {
      process.stdout.write('\033c')
      console.log(res)
