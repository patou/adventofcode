...
function isOccupied(tab, y, x) {
    return tab[y] && tab[y][x] == '#' ? 1 : 0
}
  
function countOccupied(tab, y, x) {
    return (isOccupied(tab, y - 1, x - 1) + isOccupied(tab, y - 1, x) + isOccupied(tab, y - 1, x + 1)
         + isOccupied(tab, y, x - 1)      +          0                + isOccupied(tab, y, x + 1) 
         + isOccupied(tab, y + 1, x - 1)  + isOccupied(tab, y + 1, x) + isOccupied(tab, y + 1, x + 1));
}

function stepGame1(tab) {
    let res = [];
    let changed = 0;
    let count = 0;
    ...
    return { tab: res, changed, count };
}



function run1(tab) {
    let game = { tab, changed: 1 };
    let step = 0;
    while (game.changed > 0) {
        game = stepGame1(game.tab)
        log(`---- Step ${step++} ----`);
        print(game.tab);
    }
    return game.count;
...
function isVisibleOccupied(tab, y, x, dy, dx) {
    do {
        y += dy;
        x += dx;
        if (tab[y] && tab[y][x] == '#')
...
function countVisibleOccupied(tab, y, x) {
    return (isVisibleOccupied(tab, y, x, -1, -1) + isVisibleOccupied(tab, y, x, -1, 0) + isVisibleOccupied(tab, y, x, -1, +1)
         + isVisibleOccupied(tab, y, x, 0, -1)   +          0                          + isVisibleOccupied(tab, y, x, 0, +1) 
         + isVisibleOccupied(tab, y, x, +1, -1)  + isVisibleOccupied(tab, y, x, +1, 0) + isVisibleOccupied(tab, y, x, +1, +1));
}

function stepGame2(tab) {
    let res = [];
    let changed = 0;
    let count = 0;
    for (let y = 0; y < tab.length; y++) {
        ...
        for (let x = 0; x < tab[y].length; x ++) {
            ...
            switch(tab[y][x]) {
                case 'L':
                    if (countVisibleOccupied(tab, y, x) == 0) {
                        res[y][x] = '#';
                        changed++;
                        count++;
                    }
                    break;
                case '#':
                    if (countVisibleOccupied(tab, y, x) >= 5) {
                        res[y][x] = 'L';
                        changed++;
                    }
                    else {
