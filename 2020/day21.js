...
let tab = getTab(test).split("\n").map(part => part.replace(')', '').split(' (contains ').map(part => part.split(/,? /)));

log(tab);

function run(tab) {
    let tousLesIngrediants = new Set();
    let nourritureParAllegenes = new Map();
    let allergenesPossibles = new Map();
    let nbIngrediants = new Map();
    for (let [ingrediants, allergenes] of tab) {
        for (let allergene of allergenes) {
            let list = nourritureParAllegenes.get(allergene) || [];
            list.push([...ingrediants])
            nourritureParAllegenes.set(allergene, list);
        }
        for (let ingrediant of ingrediants) {
            tousLesIngrediants.add(ingrediant);

            allergenesPossibles

            let set = allergenesPossibles.get(ingrediant) || new Set();
            for (let allergene of allergenes)
                set.add(allergene)
            allergenesPossibles.set(ingrediant, set);
            nbIngrediants.set(ingrediant, (nbIngrediants.get(ingrediant) || 0) + 1);
        }
    }
    log(nourritureParAllegenes );

    log(allergenesPossibles );
	const noAllergenes = [...tousLesIngrediants].filter(ingrediant => {
		const allergansOptions = allergenesPossibles.get(ingrediant);

		return [...allergansOptions].every(allergene => nourritureParAllegenes.get(allergene).some(ingList => !ingList.includes(ingrediant)));
	});
    log(noAllergenes);

    let part1 = noAllergenes.map(ingrediant => nbIngrediants.get(ingrediant)).reduce((a,b) => a+b, 0);

    noAllergenes.forEach(allergan => {
		allergenesPossibles.delete(allergan);
	});

    const allergenesTrouve = new Map();

	let precTaille = -1;

	while (precTaille !== allergenesTrouve.size) {
		precTaille = allergenesTrouve.size;

		for (const [ingrediant, allergenes] of allergenesPossibles.entries()) {
			if (allergenes.size === 1) {
				continue;
			}

			for (const allergan of allergenes.values()) {
				if (allergenesTrouve.has(allergan) || nourritureParAllegenes.get(allergan).some(ingList => !ingList.includes(ingrediant))) {
					allergenes.delete(allergan);
				}
			}

			if (allergenes.size === 1) {
				const allergan = allergenes.values().next().value;
				allergenesTrouve.set(allergan, ingrediant);
			}
		}
	}

	let part2 = [...allergenesTrouve.keys()]
		.sort()
		.map(allergene => allergenesTrouve.get(allergene))
		.join(',');
...
const res = run(tab);
console.log(`Result 1: ${res.part1}`);
console.log(`Result 2: ${res.part2}`);
