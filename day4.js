...
function byr(byr) {
    ...
    return year && year >= 1920 && year <= 2002;
}
...
function eyr(eyr) {
    ...
    return year && year >= 2020 && year <= 2030;
}

function hgt(hgt) {
    let res = hgt.match(/^([0-9]+)(in|cm)$/);
    if (!res) return false;
    let size = parseInt(res[1]);
    return (
        (res[2] == 'cm' && size >= 150 && size <= 193) ||
        (res[2] == 'in' && size >= 59 && size <= 76)
        );
}
...
function countValidPassport(tab, check) {
